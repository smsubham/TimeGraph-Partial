...
public class MainActivity extends AppCompatActivity {
    ...
    public class getChartData extends AsyncTask<String,Void,String>{
        ...
        protected void onPostExecute(String result) {
            ...
            //if (!result.equals("")) {
            //Log.v("","V");
            //used to separate data part from {\"device\":\"thane1\",\"sensor\":\"arduino\",\"data\":
                StringTokenizer stringTokenizer = new StringTokenizer(result, "{");

                //to move to next token as first one is \"device\":\"thane1\",\"sensor\":\"arduino\",\"data\": and we want data
                stringTokenizer.nextToken();
              // this is used to separate each data to separate token
                ...
                for (int i = 0; ; i++) {
                    ...
                    //if this is last token then it ends with }} which has to be deleted so we take substring till len-2
            ...
            for (float x =Float.parseFloat(timestamp.get(0).toString()); x < timestamp.get(timestamp.size()-1)&&i<timestamp.size()-1; x += gap) {
                ...
                gap=timestamp.get(i)-timestamp.get(i-1);//gap should vary according to gap of timestamp
                i++;
            }
        ...
        //used to convert timestamp to real date
        public void setDate(){
            for(int i=0;i<timestamp.size();i++){
                DateFormat sdf = new SimpleDateFormat("HH:mm");
                //this format gives hours of day
                ...
                //used to determine am or pm to be used
                if(k>0&&k<=11)
